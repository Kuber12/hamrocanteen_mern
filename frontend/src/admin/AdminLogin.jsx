      ...
      <div className="w-[350px] space-y-3 backdrop-blur-sm   h-[450px] rounded-3xl  shadow-lg p-6 backdrop-filter bg-opacity-20 bg-white">
        ...
        <div className="w-full h-[60%]">
          <form onSubmit={handleSubmit} className="w-full h-[70%]">
            ...
            <input onChange={(e)=>{setAdmin({...admin,username:e.target.value})}} type="text" className="px-3 pl-10 py-1 w-[15rem] rounded-2xl" placeholder="Username"/>
            </div>
            <div className="relative flex items-center justify-center w-full">
            <CiLock    className="absolute left-10" size={18}/>
            <input onChange={(e)=>{setAdmin({...admin,password:e.target.value})}} type="text" className="px-3 pl-10 py-1 w-[15rem] rounded-2xl" placeholder="Password"/>
            </div>
